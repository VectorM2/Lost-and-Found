...
function hideNonMatchingCards() {
  ...
  for (let i = 0; i < cards.length; i++) {
    ...
    if (card.style.display !== "block") {
      card.classList.add("visually-hidden"); // Add the CSS class to hide the card
    } else {
      card.classList.remove("visually-hidden"); // Remove the CSS class if the card is displayed
    }
  }
}
